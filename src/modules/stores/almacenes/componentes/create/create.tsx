...
import { warehousesCreate } from "@/shared/services/warehouse.service";

export function Create({
  handleCreate,
}: {
  handleCreate: (data: IAlmacen) => void;
}) {
  const {
    register,
    handleSubmit,
  ...
  async function handleSumit(info: IAlmacenCreate) {
    const { success, data } = await warehousesCreate(info);
