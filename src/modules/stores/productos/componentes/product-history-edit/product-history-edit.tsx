...
import styles from "./product-history-edit.module.scss";
...
import { productHistoriesUpate } from "@/shared/services/product-history.service";

export function ProductHistoryEdit({
  handleEdit,
  almacenes,
  history,
}: {
  handleEdit: (data: IProductHistory) => void;
  ...
  const {
    ...
    defaultValues: {
      productHistoryId: history.productHistoryId,
  ...
  async function handleSumit(info: IProductoHistoryUpdate) {
    const { success, data, kindMessage } = await productHistoriesUpate(info);
    if (success) handleEdit(data.item);
